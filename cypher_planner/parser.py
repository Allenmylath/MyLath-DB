...
class CypherParser:
    ...
    def _parse_pattern(self) -> Pattern:
        ...
        while True:
            ...
            # Check for incoming relationship patterns like <-[:TYPE]-
            if token == "<":
                ...
                if self._peek_token() == "-":
                    ...
                    if self._current_token() == "[":
                        rel_pattern = self._parse_relationship_brackets()
                        rel_pattern.direction = "incoming"
                        elements.append(rel_pattern)
                    else:
                        # Simple incoming relationship
                        rel_pattern = RelationshipPattern(direction="incoming")
                        elements.append(rel_pattern)
                    ...
                    # Parse target node if present
                    if self._current_token() == "(":
                        elements.append(self._parse_node_pattern())

                else:
                    # Not a relationship, break
                    break

            # Check for outgoing relationships
            elif token == "-":
                # Look ahead to parse the relationship
                if self._peek_token() == "[":
                    self._consume_token()  # consume '-'
                    rel_pattern = self._parse_relationship_brackets()
                    rel_pattern.direction = "outgoing"  # will be corrected if needed
                    elements.append(rel_pattern)

                    # Check what follows to determine final direction
                    if self._current_token() == "-":
                        self._consume_token()  # consume '-'
                        if self._current_token() == ">":
                            self._consume_token()  # consume '>'
                            rel_pattern.direction = "outgoing"
                        else:
                            rel_pattern.direction = "bidirectional"
                    ...
                    rel_pattern = RelationshipPattern(direction="bidirectional")
            ...
            # Check for direct arrows
            elif token in ["<--", "-->", "--"]:
                direction_token = self._consume_token()
                if direction_token == "<--":
                    direction = "incoming"
                elif direction_token == "-->":
                    direction = "outgoing"
                else:
                    direction = "bidirectional"

                rel_pattern = RelationshipPattern(direction=direction)
                elements.append(rel_pattern)
    ...
    def _parse_relationship_brackets(self) -> RelationshipPattern:
        """Parse relationship details within brackets [var:TYPE*1..3]"""
        self._expect_token("[")
        ...
        return RelationshipPattern(
            ...
            direction="outgoing",  # Will be set by caller
